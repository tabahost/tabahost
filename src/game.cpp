...
// WB2   WB2007  WB2008
		...
		{ 0xFFFF, 0x0009, 0x0009 }, // bgAI_UPDATE
					...
					if(k)
					{
						for(j = 0; j < MAX_HITBY; j++) // remove tonnage from bombers
						{
							if(arena->fields[i].hitby[j].dbid)
							{
								...
								{
									Com_Printf(VERBOSE_DAMAGE, "Field damage removed, remove bomber %d from list\n", arena->fields[i].hitby[j].longnick);
					...
					if(arena->fields[i].buildings[j].field)
					{
						if(arena->fields[i].buildings[j].status)
						{
							if(!ctf->value)
							{
								...
								{
									Com_Printf(VERBOSE_DAMAGE, "bugged building timer %d, set to 15min\n", arena->fields[i].buildings[j].timer);
...
void PHitPlane(u_int8_t *buffer, client_t *client)
{
	...
	for(i = 0; i < hits; i++)
	{
		...
		for(j = 0; j < 5; j++)
		{
			...
			if(damage < 0)
			{
				Com_Printf(VERBOSE_DAMAGE, "PHitPlane(damage -= ap) < 0, damage = %d, ap = %d\n", damage, ap);
				damage = 0;
			}
...
u_int16_t AddPlaneDamage(int8_t place, u_int16_t he, u_int16_t ap, char *phe, char *pap, client_t *victim, int8_t killer)
{
	...
	if(!setjmp(debug_buffer))
	{
		...
		{
			Com_Printf(VERBOSE_DAMAGE, "AddPlaneDamage(dmgprobe) out of range (%u) at line %u\n", dmgprobe, __LINE__);
			dmgprobe = 0;
		}
	}
	else
	...
	if(!(dmgprobe < victim->armor.imunity[place])) // hit makes damage
	{
		...
		if(!setjmp(debug_buffer))
		{
			...
			if(apabsorb > ap)
			{
				Com_Printf(VERBOSE_DAMAGE, "AddPlaneDamage(apabsorb) > ap at line %u (apabsorb = %d, ap = %u)\n", __LINE__, apabsorb, ap);
				apabsorb = ap;
			}
...
u_int8_t AddBuildingDamage(building_t *building, u_int16_t he, u_int16_t ap, client_t *client)
{
	...
	if(!building->field)
	{
		Com_Printf(VERBOSE_DAMAGE, "AddBuildingDamage() building %d has no field", building->id);
		return 0;
	}
